...
def split_nodes_delimiter(old_nodes, delimiter, text_type):
    ...
    for old_node in old_nodes:
        ...
        while delimiter in remaining_text:
            ...
            if start_idx > 0:
                split_parts.append((remaining_text[:start_idx], TextType.TEXT))
                
            remaining_text = remaining_text[start_idx + len(delimiter):]
            ...
            if end_idx > 0: 
                split_parts.append((remaining_text[:end_idx], text_type))
            
            remaining_text = remaining_text[end_idx + len(delimiter):]
        
        if remaining_text:
            split_parts.append((remaining_text, TextType.TEXT))
        
        for text, node_type in split_parts:
